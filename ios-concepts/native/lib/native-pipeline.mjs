...
import { assertPathInside, canonicalProjectPath, isPathInside, NATIVE_PROJECT_ROOT } from "./project-paths.mjs";

function command(label, executable, args, cwd, root) {
  assertPathInside(root, cwd, `${label} cwd`);
  for (const arg of args) {
    if (isAbsolute(arg)) assertPathInside(root, arg, `${label} argument`);
  }
  return Object.freeze({ label, executable, args: Object.freeze(args), cwd, env: Object.freeze({}) });
}

function commandWithEnv(label, executable, args, env, cwd, root) {
  const base = command(label, executable, args, cwd, root);
  return Object.freeze({ ...base, env: Object.freeze(env) });
}

export function discoverNativeConcepts(root = NATIVE_PROJECT_ROOT) {
...
function testCommands(root) {
  ...
  return [command("native tests", process.execPath, ["--test", ...tests], root, root)];
}

export function createNativePipelinePlan(operation, slug, options = {}) {
  const root = options.projectRoot || NATIVE_PROJECT_ROOT;
  const native = join(root, "native");
  const productGate = target => command(`product maturity ${target}`, process.execPath, [join(native, "gen", "gate-product.mjs"), target], root, root);
  const compile = target => command(`compile ${target}`, process.execPath, [join(native, "gen", "compile-concept.mjs"), target, "--write"], root, root);
  const docs = target => command(`developer docs ${target}`, process.execPath, [join(native, "gen", "developer-docs.mjs"), target, "--check"], root, root);
  const delivery = target => command(`native delivery ${target}`, process.execPath, [join(native, "gen", "gate-delivery.mjs"), target], root, root);
  const generate = target => command(`generate ${target}`, process.execPath, [join(native, "gen", "gen-project.mjs"), target], root, root);
  const audit = target => command(`audit ${target}`, process.execPath, [join(native, "gen", "audit-native.mjs"), target], root, root);
  const build = target => {
    const name = target[0].toUpperCase() + target.slice(1);
    const directory = join(native, "build", target);
    return command(`build ${target}`, "/usr/bin/xcodebuild", ["-project", join(directory, `${name}.xcodeproj`), "-target", name, "-sdk", "iphonesimulator", "-configuration", "Debug", "build"], directory, root);
  };
  const capture = target => command(`capture ${target}`, process.execPath, [join(native, "gen", "shots.mjs"), target], root, root);
  const critic = target => command(`critic ${target}`, process.execPath, [join(native, "gen", "critic.mjs"), target], root, root);
...
export function runNativePipeline({ operation, slug, adapter, root = NATIVE_PROJECT_ROOT }) {
  ...
  if (!adapter) {
    ...
    writeFileSync(join(receiptDirectory, `${id}.json`), JSON.stringify({
      ...
      steps: steps.map(step => ({
        label: step.label,
        executable: step.executable === process.execPath ? "$NODE" : step.executable,
        args: step.args.map(arg => isAbsolute(arg) && isPathInside(root, arg) ? canonicalProjectPath(root, arg) : arg),
        cwd: canonicalProjectPath(root, step.cwd),
        env: step.env,
      })),
    }, null, 2) + "\n");
  }
...
export { NATIVE_PROJECT_ROOT } from "./project-paths.mjs";
