...
import { compileNativeFullBlueprintV2, compileNativeKernelV2, compileNativeSliceBlueprintV2 } from "./native-kernel-v2.mjs";
...
function assertOwnedOrEmpty(appDirectory, slug) {
  ...
  const marker = join(appDirectory, ".native-pipeline-v2.json");
  if (!existsSync(marker)) throw new Error(`Refusing to overwrite existing app ${slug}: it is not owned by Native Pipeline v2`);
...
export function materializeNativeSliceV2({
  projectRoot,
  productCore,
  capabilityPlan,
  sliceContract,
  ...
  const blueprint = compileNativeSliceBlueprintV2({
    productCoreArtifact: productCore, capabilityPlan, sliceContract, targetProduct, strategy,
  });
  const compiled = compileNativeConcept(blueprint, { bundleId: `com.camo.${blueprint.id.replace(/[-_]/g, "")}` });
  if (!compiled.ok) throw new Error(compiled.diagnostics.map(item => `${item.code}: ${item.message}`).join("\n"));

  const kernel = compileNativeKernelV2({ productCoreArtifact: productCore, capabilityPlan, sliceContract });
  const capture = compileCaptureCatalog(compiled.manifest, kernel.captureCatalog);
  if (!capture.ok || capture.missing.length) {
    const messages = [
  ...
  json(join(appDirectory, "product-state-surfaces.json"), {
    product: sliceContract.surfaces.map(surface => surface.id), pendingProduct: [], system: ["login"], extensionSurface: [],
  });
  json(join(appDirectory, ".native-pipeline-v2.json"), {
...
export function materializeNativeFullV2({
  projectRoot,
  productCore,
  capabilityPlan,
  fullContract,
...
export function executeNativeSliceV2({ projectRoot, materialized, simulator = SIMULATOR }) {
  const root = resolve(projectRoot);
  const nativeRoot = join(root, "native");
  ...
  const derivedData = join(nativeRoot, "artifacts", slug, "NativeV2DerivedData");
...
export function createNativeKernelAdapterV2({ projectRoot, executor = executeNativeSliceV2, simulator = SIMULATOR }) {
  const root = resolve(projectRoot);
  return Object.freeze({
    async buildSlice(input) {
      try {
        const materialized = materializeNativeSliceV2({ projectRoot: root, ...input });
        return await executor({ projectRoot: root, materialized, simulator });
      } catch (error) {
        const message = String(error?.message || error);
        return Object.freeze({
          buildReceipt: Object.freeze({ passed: false, error: message }),
          interactionReceipt: Object.freeze({ passed: false, error: message }),
          documentationReceipt: Object.freeze({ passed: false, error: message }),
    ...
    async buildFull(input) {
      try {
        const materialized = materializeNativeFullV2({ projectRoot: root, ...input });
        return await executor({ projectRoot: root, materialized, simulator });
      } catch (error) {
        const message = String(error?.message || error);
        return Object.freeze({
...
export function listNativeV2OwnedFiles(appDirectory) {
  if (!existsSync(appDirectory)) return [];
  return readdirSync(appDirectory, { recursive: true, withFileTypes: true })
