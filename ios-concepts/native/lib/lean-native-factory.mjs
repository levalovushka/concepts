...
export async function runLeanNativeFactory({ request, architect, builder, reviewer, onProgress = null }) {
  const target = resolveProductTarget(request?.targetProduct);
  if (!target) return { ok: false, stage: "request", diagnostics: [diagnostic("request.target", "Unknown target product", "targetProduct")], artifacts: {} };
  if (!architect?.design || !builder?.build || !reviewer?.review) return {
    ok: false, stage: "request", diagnostics: [diagnostic("factory.adapters", "Architect, builder and reviewer adapters are required", "adapters")], artifacts: {},
  };
  ...
  const stage = async (id, operation) => {
    ...
    measurements.push({ id, durationMs });
  ...
  const calibration = resolveVisualCalibration({ strategy: request.strategy, referenceProfileId: reference?.id || null });
  const blueprint = await stage("product-blueprint", () => architect.design({ request: structuredClone(request), target, reference }));
  const blueprintDiagnostics = verifyProductBlueprint(blueprint, request, target);
  artifacts.productBlueprint = blueprint;
  if (blueprintDiagnostics.length) return { ok: false, stage: "product-blueprint", diagnostics: blueprintDiagnostics, artifacts, measurements };
  const delivery = await stage("native-build", () => builder.build({ blueprint: structuredClone(blueprint), target, reference, calibration }));
  artifacts.delivery = delivery;
  if (!delivery?.buildReceipt?.passed || delivery?.interactionReceipt?.passed !== true
      || delivery?.documentationReceipt?.passed !== true
      || !Array.isArray(delivery?.captures) || delivery.captures.length < blueprint.navigation.screens.length) return {
    ok: false, stage: "native-build", diagnostics: [diagnostic("delivery.proof", "Handoff needs build, interaction and split developer-documentation receipts plus a real default capture for every screen", "delivery")], artifacts, measurements,
  };
  const review = await stage("delivery-review", () => reviewer.review({ blueprint: structuredClone(blueprint), delivery: structuredClone(delivery), reference, calibration }));
  artifacts.review = review;
  if (review?.passed !== true || (review?.blockers || []).length) return {
    ok: false, stage: "delivery-review", diagnostics: (review?.blockers || ["Independent review did not pass"]).map((message, index) => diagnostic("review.blocker", message, `review.blockers[${index}]`)), artifacts, measurements,
  };
  return { ok: true, stage: "complete", diagnostics: [], artifacts, measurements };
}
