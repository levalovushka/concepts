...
import { executeNativeConcept, materializeNativeConcept } from "./native-delivery.mjs";
...
export function verifyNativeConceptSpec(spec) {
  const diagnostics = [];
...
export function generateNativeConcept({ projectRoot, spec, execute = true, simulator, visualReview = null }) {
  ...
  const materialized = materializeNativeConcept({
    projectRoot,
  ...
  const delivery = execute ? executeNativeConcept({ projectRoot, materialized, simulator }) : null;
