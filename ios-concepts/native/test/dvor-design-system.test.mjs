...
test("Dvor keeps the native system tab bar for Liquid Glass", () => {
  ...
  assert.match(source["App.swift"], /DvorLockScreen\(unlock: unlock\)[\s\S]*?\.environment\(\\\.visualLanguage, visualLanguage\)/,
    "the lock overlay must inherit VK mimicry instead of the default purple product palette");
});

test("Dvor menu has one profile entry and headers have no decorative chevrons", () => {
  const menu = source["Tabs.swift"].slice(source["Tabs.swift"].indexOf("struct HouseMenuScreen"));
  assert.doesNotMatch(menu, /title: "Меню",\s*avatar:/);
  assert.equal((menu.match(/DvorRoute\.profile/g) || []).length, 1);
  assert.doesNotMatch(allSwift, /dropdown:\s*true/);
});
...
test("Dvor optical gate keeps critical geometry on a four-point grid", () => {
  ...
  const home = source["Home.swift"];
  assert.doesNotMatch(allSwift, /\bDvorStyle\b/);
  assert.match(ds, /@Environment\(\\\.visualLanguage\) private var t/);
  assert.match(ds, /t\.spacing\.x1/);
  assert.match(ds, /t\.metrics\.hitTarget/);
  assert.match(home, /t\.spacing\.contentInset/);
  assert.match(home, /frame\(minHeight: t\.metrics\.hitTarget\)/);
  assert.match(home, /LazyVStack\(spacing: t\.spacing\.sectionGap\)/);
});
