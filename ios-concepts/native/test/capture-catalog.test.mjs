...
const authSource = readFileSync(join(nativeRoot, "DesignSystem/NativeEmailAuth.swift"), "utf8");
...
test("migrated app surfaces keep their own chrome and consume product state", () => {
  ...
  assert.match(appSource, /NativeEmailAuth\(\s*productName: "Образы"/);
  assert.match(authSource, /captureState == "loading"/);
  assert.match(authSource, /captureState == "error"/);
  assert.match(authSource, /NativeOTPField\(code: \$code, length: codeLength/);
  assert.doesNotMatch(chatsSource, /struct AuthScreen/);
