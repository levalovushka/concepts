...
function concept(actions, primaryAction = "Открыть дело") {
  return {
    ...
    native: { actionContractVersion: 3 },
    screens: [{ id: "home", ui: { primaryAction, actions: actions.map(action => ({
      variant: "primary", placement: "body", ...action,
    })) } }],
  };
}
  ...
  const result = compileActionContracts(concept([
    {
      ...
      failure: { message: "Не удалось открыть дело.", retry: true,
        fallback: { type: "manual", action: "keep-working-offline" } },
    },
  ]), [surface], []);
...
test("primary actions cannot drift into toolbar placement", () => {
  const result = compileActionContracts(concept([{
    id: "open", label: "Открыть дело",
    outcome: { type: "navigate", target: "home" }, execution: "sync",
    persistence: "none", states: ["idle", "success"], placement: "toolbar",
  }]), [surface], []);
  assert.equal(result.diagnostics.some(item => item.code === "action.placement.incompatible"), true);
});

test("async adapter success may declare its persisted product mutation", () => {
  const result = compileActionContracts(concept([{
    id: "open", label: "Открыть дело",
    outcome: { type: "request", capability: "wifiinfo" }, execution: "async",
    persistence: "system", states: ["idle", "loading", "success", "error"],
    failure: { message: "Нет сети.", retry: true,
      fallback: { type: "manual", action: "ask-organizer" } },
    successOutcome: { type: "mutate", state: "checkedIn", persistence: "local" },
  }]), [surface], [{ key: "wifiinfo" }]);
  assert.equal(result.diagnostics.length, 0);
});
