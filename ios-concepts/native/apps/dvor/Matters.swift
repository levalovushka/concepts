                        ...
                        ForEach(matter.replies) { reply in
                            HStack(alignment: .top, spacing: 10) {
                                ...
                                VStack(alignment: .leading, spacing: 4) {
                                    ...
                                    Text(reply.text).font(.vkBody).foregroundStyle(t.palette.textPrimary)
                                    Text(reply.time).font(.vkMeta).foregroundStyle(t.palette.textSecondary)
                                }
                            }
                        }
                    }
                }
                .padding(.horizontal, t.spacing.contentInset).padding(.bottom, 18)
                GroupGap()
                VStack(spacing: 10) {
                    ...
                    HStack(spacing: 10) {
                        TextField("Написать комментарий", text: $replyText)
                            .padding(.horizontal, 14).frame(height: 44).background(t.palette.fill, in: Capsule())
                        Button {
                    ...
                    if DvorShotMode.state == "success" || createdMatterID != nil {
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            TextEditor(text: $details).font(.role(.body))
                                ...
                                .background(t.palette.fill, in: RoundedRectangle(cornerRadius: 12))
                        }
                        ...
                        PhotosPicker(selection: $evidenceItem, matching: .images) {
                            HStack(spacing: 12) {
                                ...
                                VStack(alignment: .leading, spacing: 2) {
                                    Text(evidenceData == nil ? "Приложить фото" : "Заменить фото").font(.role(.body))
                                    Text("Поможет быстрее найти место и проверить проблему").font(.vkMeta).foregroundStyle(t.palette.textSecondary)
                                }
                                Spacer()
                            }
                            .foregroundStyle(t.palette.textPrimary).padding(14)
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 18) {
                Text("Выберите снимки двора для личной хроники. Они останутся на устройстве, пока вы сами не поделитесь ими с соседями.")
                    .font(.vkBody).foregroundStyle(t.palette.textSecondary).lineSpacing(3)
                ...
                PhotosPicker(selection: $selectedPhotos, maxSelectionCount: 4, matching: .images) {
                    Text(selectedData.isEmpty ? "Выбрать из медиатеки" : "Изменить выбор")
                        .font(.role(.cardTitle)).foregroundStyle(.white)
                        .frame(maxWidth: .infinity, minHeight: 48).background(t.palette.accent, in: RoundedRectangle(cornerRadius: 10))
                }
