...
struct ResidenceVerificationScreen: View {
    ...
    var body: some View {
        DvorEntryScaffold { DvorRootChrome(title: "Двор") } content: {
            VStack(alignment: .leading, spacing: 16) {
                ...
                .background(t.palette.background, in: RoundedRectangle(cornerRadius: 12, style: .continuous))
    ...
    private func verificationRow(number: String, title: String, detail: String, complete: Bool) -> some View {
        HStack(alignment: .top, spacing: 12) {
            Group {
                if complete {
                    Image(systemName: "checkmark.circle.fill").font(.system(size: 22, weight: .semibold)).foregroundStyle(t.palette.accent)
                } else {
                    Text(number).font(.role(.name)).foregroundStyle(t.palette.accent)
                }
            }.frame(width: 32, height: 32)
            VStack(alignment: .leading, spacing: 3) {
                Text(title).font(.role(.name))
                Text(detail).font(.role(.meta)).foregroundStyle(t.palette.textSecondary).fixedSize(horizontal: false, vertical: true)
            }
            Spacer(minLength: 0)
        }.padding(12)
    }
