...
struct MeterScreen: View {
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                if submitted {
                    ...
                    .overlay(RoundedRectangle(cornerRadius: 12).stroke(t.palette.separator))
                ...
                DvorPrimaryButton(title: "Сохранить показания", loadingTitle: "Сохраняем…", isLoading: isSubmitting) {
                    ...
                    Text("Прошлые показания: 112 · 73 · 3744").font(.vkMeta).foregroundStyle(t.palette.textSecondary)
                }
            }.padding(t.spacing.contentInset)
    ...
    private func meterField(_ title: String, unit: String, text: Binding<String>) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack {
                ...
                Text(unit).font(.vkBody).foregroundStyle(t.palette.textSecondary)
            }.padding(.horizontal, 12).frame(height: 46).background(t.palette.fill, in: RoundedRectangle(cornerRadius: t.metrics.controlRadius))
        }
    }
    ...
    private func meterReceiptRow(_ title: String, value: String) -> some View {
        HStack {
            Text(title).font(.role(.action))
            Spacer()
            Text(value).font(.role(.body)).foregroundStyle(t.palette.textSecondary)
        }
...
struct GuestAccessScreen: View {
    ...
    var body: some View {
        ...
        .padding(t.spacing.contentInset).background(t.palette.surface).vkNavigation("Гостевая сеть")
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if let scannedCode {
                ...
                VStack(spacing: 16) {
                    Image(systemName: "camera.fill").font(.system(size: 32)).foregroundStyle(t.palette.accent)
                    Text("Сканер недоступен на этом устройстве").font(.role(.navTitle))
                    Text("Откройте пароль гостевой сети и подключитесь вручную.")
                        .font(.role(.body)).foregroundStyle(t.palette.textSecondary).multilineTextAlignment(.center)
            ...
            VStack(spacing: 0) {
                ...
                ForEach(store.events) { event in
                    HStack(alignment: .top, spacing: 14) {
                        Text(event.day).font(.role(.groupHeader)).foregroundStyle(t.palette.accent).frame(width: 56, alignment: .leading)
                        VStack(alignment: .leading, spacing: 4) {
                            Text(event.title).font(.role(.cardTitle))
                            Text(event.detail).font(.vkMeta).foregroundStyle(t.palette.textSecondary)
        ...
        ScrollView {
            VStack(spacing: 0) {
                ...
                Text("Коды защищены блокировкой устройства и доступны системному AutoFill. Двор не показывает их неподтверждённым жильцам.")
                    .font(.vkMeta).foregroundStyle(t.palette.textSecondary).padding(t.spacing.contentInset)
                if let accessCheck {
                    ...
                    .padding(.horizontal, t.spacing.contentInset).padding(.bottom, 12)
                }
    ...
    var body: some View {
        ScrollView {
            LazyVStack(spacing: 0) {
                ...
                }.padding(.horizontal, t.spacing.contentInset).padding(.bottom, 8).disabled(isMatching)
                    ...
                    VKPersonRow(name: person.name, subtitle: [person.apartment, person.role].compactMap { $0 }.joined(separator: " · ")) {
                        if matchedNames.contains(person.name) {
                            Image(systemName: "person.crop.circle.badge.checkmark")
                                .foregroundStyle(t.palette.accent).frame(width: 32, height: 44)
...
struct DvorSettingsScreen: View {
    ...
    var body: some View {
            ...
            LazyVStack(spacing: t.spacing.sectionGap) {
                ...
                DvorCard {
                    VStack(spacing: 0) {
                        ...
                        DvorRow(title: "Обновлять дом в фоне", subtitle: "Актуальные дела и сообщения после открытия", toggle: $backgroundUpdates)
        ...
        .onChange(of: backgroundUpdates) { _, enabled in
            guard !isHydrating, enabled else { return }
            Task {
                let granted = await permissions.request(.bgtask)
                if !granted { backgroundUpdates = false }
                nav.toast(granted ? "Фоновое обновление включено" : "Дом обновится при открытии приложения")
            }
        }
        .task {
            isHydrating = false
