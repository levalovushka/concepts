...
| nearby | Найти обмен вещами или автора поблизости | push | default<br>empty | Смотреть подборку → navigate:event<br>Разрешить геопозицию → request |
| clip | Посмотреть серию | tab | default | Сохранить клип → mutate:saved |
...
| event | Проверить условия встречи для обмена вещами и присоединиться | push | available<br>joined<br>cancelled | Присоединиться → mutate:joinedEvents |
...
| swap | Проверить две вещи, предложить обмен и сохранить встречу | push | default | Добавить в Календарь → request |
| checkin | Подтвердить присутствие сетью площадки | push | default<br>error<br>denied | Отметиться на встрече → request |
