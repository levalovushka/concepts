...
| `com.apple.developer.usernotifications.communication` | «Изменения приёма» | Прививки и обработки | Новая дата остаётся в ветпаспорте и обычном уведомлении | **Условный** — INSendMessageIntent только для структурированного изменения записи |
| `UIBackgroundModes: voip` | «Видеоосмотр с Марией Тенищевой» | Прививки и обработки | Можно приложить видео наблюдения к записи на очный приём | **Условный** — CallKit + WebRTC только для подтверждённого приёма |
<!-- @end -->
...
Вход по номеру (phone) — старт, без таб-бара · открывается: старт
    ...
    └─ Главная (home) — tab (root) · открывается: «Продолжить без аккаунта», «Войти» …, «Открыть «Хвосты»» (keychain), «Проверить задачу» (bgtask) · location
        ...
        │   └─ Наблюдение владельца (vetnote) — push · открывается: «Надиктовать заметку» (speech), «Надиктовать заметку о» (mic) · mic
