...
Вход по почте или Google (phone) — старт, без таб-бара · открывается: старт
    └─ Код из письма (code) — push · открывается: «Продолжить с почтой»
        ...
        └─ Найдите свой дом (join) — push · открывается: «Продолжить с Google», «Продолжить» · location
...
| `phone` | Продолжить с Google | элемент экрана | открывает экран | Найдите свой дом (join) | — | — |
| `phone` | Помощь | элемент экрана | показывает подтверждение «Помощь · dvor.app/help» | остаётся на экране | — | — |
| `phone` | Поддержка | элемент экрана | показывает подтверждение «Поддержка · support@dvor.app» | остаётся на экране | — | — |
| `phone` | Пользовательское соглашение | элемент экрана | показывает подтверждение «Соглашение · dvor.app/terms» | остаётся на экране | — | — |
| `code` | Назад к номеру | кнопка «назад» | возвращает к родительскому экрану | Вход по почте или Google (phone) | — | — |
| `code` | Ввести код | элемент экрана | открывает экран | Неверный код (codefail) | — | — |
| `code` | Продолжить | элемент экрана | открывает экран | Найдите свой дом (join) | — | — |
