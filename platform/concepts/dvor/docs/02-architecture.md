...
Вход по номеру (phone) — старт, без таб-бара · открывается: старт
    └─ Код из SMS (code) — push · открывается: «Получить код»
        └─ Найдите свой дом (join) — push · открывается: «Продолжить» · location
            ├─ Проверка сети (verify) — modal · открывается: «Я дома — проверить» (location), «Проверить домашнюю сеть заново» · wifiinfo
            └─ Адрес вручную (manual) — push · открывается: «Я дома — проверить» (location), «Ввести адрес вручную»

Дом (home) — tab (root) · открывается: «Подтвердить дом» (wifiinfo), «Отправить на проверку», «Открыть Двор из виджета» (keychain) · photos
    ...
    │   └─ Камера (shoot) — системная поверхность · открывается: «Снять, как стоит» (camera)
...
Меню (menu) — tab (root) · открывается: «Продолжить» (tracking), «Разблокировать Двор по Face ID» · contacts
    ...
    └─ Настройки (settings) — push · открывается: «Настройки», «Настройки и доступы» · fetch, appgroups, faceid
...
| `join` | «Ввести адрес вручную» | `manual` | — | переход |
...
| `manual` | «Отправить на проверку» | `home` | — | переход |
...
| `problem` | «Снять, как стоит» | `shoot` | `NSCameraUsageDescription` | доступ разрешён |
| `problem` | «Снять, как стоит» | `problem` | `NSCameraUsageDescription` | отказ → fallback |
...
| `menu` | «Настройки», «Настройки и доступы» | `settings` | — | переход |
