...
1. Первый экран — всегда **регистрация по почте** (`phone` → `join`), **без** OTP, внешних провайдеров и permission-алертов. Ни один доступ не спрашивается до того, как житель попал на `join`.
...
| `guest` | «Сканировать QR с лавочки» | `scan` | — | переход |
...
| `phone` | Продолжить без аккаунта | элемент экрана | открывает экран | Найдите свой дом (join) | — | — |
| `phone` | пользовательское соглашение | элемент экрана | показывает подтверждение «Соглашение · dvor.app/terms» | остаётся на экране | — | — |
| `phone` | политику конфиденциальности | элемент экрана | показывает подтверждение «Политика · dvor.app/privacy» | остаётся на экране | — | — |
| `phone` | Помощь и поддержка | элемент экрана | показывает подтверждение «Справка · dvor.app/help · support@dvor.app» | остаётся на экране | — | — |
| `password` | Назад | кнопка «назад» | возвращает к родительскому экрану | Вход по номеру (phone) | — | — |
| `password` | Войти | элемент экрана | открывает экран | Найдите свой дом (join) | — | — |
...
| `registerpassword` | Создать аккаунт | элемент экрана | открывает экран | Найдите свой дом (join) | — | — |
| `registerpassword` | пользовательское соглашение | элемент экрана | показывает подтверждение «Соглашение · dvor.app/terms» | остаётся на экране | — | — |
| `registerpassword` | политику конфиденциальности | элемент экрана | показывает подтверждение «Политика · dvor.app/privacy» | остаётся на экране | — | — |
| `registerpassword` | Помощь и поддержка | элемент экрана | показывает подтверждение «Справка · dvor.app/help · support@dvor.app» | остаётся на экране | — | — |
...
| `join` | Я дома — проверить | элемент экрана | спрашивает доступ location | Проверка сети (verify) | Адрес вручную (manual) | `NSLocationWhenInUseUsageDescription` |
| `join` | Моего дома нет в списке | элемент экрана | открывает экран | Адрес вручную (manual) | — | — |
| `verify` | Назад к выбору дома | кнопка «назад» | возвращает к родительскому экрану | Найдите свой дом (join) | — | — |
| `verify` | Подтвердить дом | элемент экрана | включает entitlement wifiinfo, системного alert нет | Дом (home) | — | `com.apple.developer.networking.wifi-info` |
| `manual` | Назад к выбору дома | кнопка «назад» | возвращает к родительскому экрану | Найдите свой дом (join) | — | — |
| `manual` | Добавить дом | элемент экрана | открывает экран | Дом (home) | — | — |
| `home` | Сообщить о проблеме | элемент экрана | открывает экран | Сообщить о проблеме (problem) | — | — |
| `home` | Что нового в доме | элемент экрана | открывает экран | Объявление (post) | — | — |
| `home` | Собрать хронику двора | элемент экрана | спрашивает доступ photos | Хроника двора (chronicle) | Дом (home) | `NSPhotoLibraryUsageDescription` |
| `home` | Объявление: отключение горячей воды | элемент экрана | открывает экран | Объявление (post) | — | — |
| `home` | Обсуждение | элемент экрана | открывает экран | Объявление (post) | — | — |
| `home` | Оформить заявку по этой проблеме | элемент экрана | открывает экран | Сообщить о проблеме (problem) | — | — |
| `home` | Открыть хронику двора | элемент экрана | открывает экран | Хроника двора (chronicle) | — | — |
| `home` | Раздел Дом | вкладка | переключает вкладку таб-бара | Дом (home) | — | — |
| `home` | Раздел Чаты | вкладка | переключает вкладку таб-бара | Чаты (chats) | — | — |
| `home` | Раздел Двор | вкладка | переключает вкладку таб-бара | Двор (yard) | — | — |
| `home` | Раздел Меню | вкладка | переключает вкладку таб-бара | Меню (menu) | — | — |
| `post` | Назад в ленту дома | кнопка «назад» | возвращает к родительскому экрану | Дом (home) | — | — |
| `post` | Следить за темой | элемент экрана | спрашивает доступ push | Объявление (post) | Объявление (post) | `aps-environment` |
| `post` | Пожаловаться на объявление | элемент экрана | показывает подтверждение «Жалоба отправлена старшему по дому» | остаётся на экране | — | — |
| `problem` | Закрыть заявку | кнопка «назад» | возвращает к родительскому экрану | Дом (home) | — | — |
| `problem` | Фото с места | элемент экрана | спрашивает доступ camera | Камера (shoot) | Сообщить о проблеме (problem) | `NSCameraUsageDescription` |
| `problem` | Отправить заявку | элемент экрана | показывает подтверждение «Заявка 4417-Б отправлена в УК» | остаётся на экране | — | — |
| `shoot` | Закрыть камеру | кнопка «назад» | возвращает к родительскому экрану | Сообщить о проблеме (problem) | — | — |
| `shoot` | Снять кадр | элемент экрана | показывает подтверждение «Кадр добавлен к заявке» | Сообщить о проблеме (problem) | — | — |
| `chronicle` | Назад в ленту дома | кнопка «назад» | возвращает к родительскому экрану | Дом (home) | — | — |
| `chronicle` | Добавить кадры в хронику двора | элемент экрана | показывает подтверждение «6 кадров добавлено в хронику двора» | остаётся на экране | — | — |
| `chats` | Чат третьего подъезда, Полевая 12 | элемент экрана | открывает экран | Чат подъезда (chat) | — | — |
| `chats` | Чат всего дома | элемент экрана | открывает экран | Чат подъезда (chat) | — | — |
| `chats` | Раздел Дом | вкладка | переключает вкладку таб-бара | Дом (home) | — | — |
| `chats` | Раздел Чаты | вкладка | переключает вкладку таб-бара | Чаты (chats) | — | — |
| `chats` | Раздел Двор | вкладка | переключает вкладку таб-бара | Двор (yard) | — | — |
| `chats` | Раздел Меню | вкладка | переключает вкладку таб-бара | Меню (menu) | — | — |
| `chat` | Назад к чатам | кнопка «назад» | возвращает к родительскому экрану | Чаты (chats) | — | — |
| `chat` | Кто в подъезде | элемент экрана | открывает экран | Профиль соседа (profile) | — | — |
| `chat` | Пожаловаться на сообщение | элемент экрана | показывает подтверждение «Жалоба отправлена старшему по подъезду» | остаётся на экране | — | — |
...
| `chat` | Записать голосовое | элемент экрана | спрашивает доступы цепочкой: mic → speech | Голосовое (voice) | Чат подъезда (chat) | `NSMicrophoneUsageDescription + NSSpeechRecognitionUsageDescription` |
| `voice` | Закрыть запись | кнопка «назад» | возвращает к родительскому экрану | Чат подъезда (chat) | — | — |
| `voice` | Свернуть | кнопка «назад» | возвращает к родительскому экрану | Чат подъезда (chat) | — | — |
| `voice` | Отменить запись | кнопка «назад» | возвращает к родительскому экрану | Чат подъезда (chat) | — | — |
| `voice` | Отменить голосовое | кнопка «назад» | возвращает к родительскому экрану | Чат подъезда (chat) | — | — |
| `voice` | Отправить голосовое | элемент экрана | показывает подтверждение «Голосовое отправлено · 0:11» | Чат подъезда (chat) | — | — |
| `lockscreen` | Открыть чат подъезда | элемент экрана | открывает экран | Чат подъезда (chat) | — | — |
| `yard` | Сообщить о проблеме во дворе | элемент экрана | открывает экран | Сообщить о проблеме (problem) | — | — |
| `yard` | Гостевая сеть двора | элемент экрана | открывает экран | Гостевая сеть (guest) | — | — |
| `yard` | Счётчики | элемент экрана | открывает экран | Счётчики (meters) | — | — |
| `yard` | События дома | элемент экрана | открывает экран | События дома (events) | — | — |
| `yard` | Раздел Дом | вкладка | переключает вкладку таб-бара | Дом (home) | — | — |
| `yard` | Раздел Чаты | вкладка | переключает вкладку таб-бара | Чаты (chats) | — | — |
| `yard` | Раздел Двор | вкладка | переключает вкладку таб-бара | Двор (yard) | — | — |
| `yard` | Раздел Меню | вкладка | переключает вкладку таб-бара | Меню (menu) | — | — |
| `guest` | Назад во двор | кнопка «назад» | возвращает к родительскому экрану | Двор (yard) | — | — |
| `guest` | Сканировать QR с лавочки | элемент экрана | открывает экран | Сканер QR (scan) | — | — |
| `guest` | Подключиться к Dvor-Guest | элемент экрана | спрашивает доступ hotspot | Гостевая сеть (guest) | Гостевая сеть (guest) | `com.apple.developer.networking.HotspotConfiguration` |
| `scan` | Закрыть сканер | кнопка «назад» | возвращает к родительскому экрану | Гостевая сеть (guest) | — | — |
| `scan` | Распознано: Dvor-Guest | элемент экрана | показывает подтверждение «Сеть Dvor-Guest распознана» | Гостевая сеть (guest) | — | — |
| `meters` | Назад к двору | кнопка «назад» | возвращает к родительскому экрану | Двор (yard) | — | — |
| `meters` | Обновлять показания и срок в фоне | элемент экрана | включает entitlement remotenotif, системного alert нет | Обновление в фоне (background) | — | `UIBackgroundModes: remote-notification` |
| `meters` | Сохранить показания | элемент экрана | показывает подтверждение «Показания сохранены на устройстве» | остаётся на экране | — | — |
| `background` | Назад к счётчикам | кнопка «назад» | возвращает к родительскому экрану | Счётчики (meters) | — | — |
| `background` | Проверить фоновую задачу | элемент экрана | включает entitlement bgtask, системного alert нет | Счётчики (meters) | — | `BGTaskSchedulerPermittedIdentifiers` |
| `events` | Назад к двору | кнопка «назад» | возвращает к родительскому экрану | Двор (yard) | — | — |
| `events` | Добавить субботник в Календарь | элемент экрана | спрашивает доступ calendar | События дома (events) | События дома (events) | `NSCalendarsFullAccessUsageDescription` |
| `menu` | Настройки | элемент экрана | открывает экран | Настройки (settings) | — | — |
| `menu` | Мой профиль жильца | элемент экрана | открывает экран | Профиль соседа (profile) | — | — |
| `menu` | Соседи из контактов | элемент экрана | спрашивает доступ contacts | Соседи из контактов (neighbors) | Соседи из контактов (neighbors) | `NSContactsUsageDescription` |
| `menu` | Счётчики | элемент экрана | открывает экран | Счётчики (meters) | — | — |
| `menu` | События дома | элемент экрана | открывает экран | События дома (events) | — | — |
| `menu` | Пароли дома | элемент экрана | открывает экран | Пароли дома (passwords) | — | — |
| `menu` | Гостевая сеть | элемент экрана | открывает экран | Гостевая сеть (guest) | — | — |
| `menu` | Хроника двора | элемент экрана | открывает экран | Хроника двора (chronicle) | — | — |
| `menu` | Чаты подъездов | элемент экрана | открывает экран | Чаты (chats) | — | — |
| `menu` | Реклама местных услуг | элемент экрана | открывает экран | Реклама (ads) | — | — |
| `menu` | Схема двора | элемент экрана | открывает экран | Двор (yard) | — | — |
| `menu` | Настройки | элемент экрана | открывает экран | Настройки (settings) | — | — |
| `menu` | Раздел Дом | вкладка | переключает вкладку таб-бара | Дом (home) | — | — |
| `menu` | Раздел Чаты | вкладка | переключает вкладку таб-бара | Чаты (chats) | — | — |
| `menu` | Раздел Двор | вкладка | переключает вкладку таб-бара | Двор (yard) | — | — |
...
| `passwords` | Включить автозаполнение паролей дома | элемент экрана | включает entitlement autofill, системного alert нет | Автозаполнение в Safari (fill) | — | `com.apple.developer.authentication-services.autofill-credential-provider` |
| `passwords` | Копировать пароль кабинета УК | элемент экрана | показывает подтверждение «Пароль скопирован» | остаётся на экране | — | — |
| `passwords` | Копировать пароль видеонаблюдения | элемент экрана | показывает подтверждение «Пароль скопирован» | остаётся на экране | — | — |
| `passwords` | Копировать пароль гостевой сети | элемент экрана | показывает подтверждение «Пароль скопирован» | остаётся на экране | — | — |
| `fill` | Вернуться в приложение «Двор» | кнопка «назад» | возвращает к родительскому экрану | Пароли дома (passwords) | — | — |
| `neighbors` | Назад в меню | кнопка «назад» | возвращает к родительскому экрану | Меню (menu) | — | — |
| `neighbors` | Профиль Петра Ильина, кв. 12 | элемент экрана | открывает экран | Профиль соседа (profile) | — | — |
| `neighbors` | Профиль Марины Кольцовой, кв. 48 | элемент экрана | открывает экран | Профиль соседа (profile) | — | — |
| `neighbors` | Профиль Ирины Тепляк, кв. 51 | элемент экрана | открывает экран | Профиль соседа (profile) | — | — |
| `profile` | Назад в меню | кнопка «назад» | возвращает к родительскому экрану | Меню (menu) | — | — |
| `profile` | Написать Петру в чат подъезда | элемент экрана | открывает экран | Чат подъезда (chat) | — | — |
| `profile` | Позвонить в квартиру 12 | элемент экрана | включает entitlement voip, системного alert нет | Звонок в квартиру (call) | — | `UIBackgroundModes: voip` |
| `profile` | Пожаловаться на жильца | элемент экрана | показывает подтверждение «Жалоба отправлена старшему по дому» | остаётся на экране | — | — |
| `profile` | Заблокировать Петра Ильина | элемент экрана | показывает подтверждение «Пётр Ильин заблокирован — его сообщения скрыты» | остаётся на экране | — | — |
| `call` | Завершить вызов | элемент экрана | показывает подтверждение «Вызов завершён · 0:42» | Профиль соседа (profile) | — | — |
...
| `settings` | Проверить домашнюю сеть заново | элемент экрана | открывает экран | Проверка сети (verify) | — | — |
...
| `settings` | Виджет на экран «Домой» | элемент экрана | включает entitlement appgroups, системного alert нет | Виджет на экране «Домой» (widget) | — | `com.apple.security.application-groups` |
| `settings` | Замок Face ID на приложении | элемент экрана | спрашивает доступ faceid | Замок Face ID (lock) | Настройки (settings) | `NSFaceIDUsageDescription` |
| `settings` | Реклама и отслеживание | элемент экрана | открывает экран | Реклама (ads) | — | — |
| `ads` | Закрыть экран про рекламу | кнопка «назад» | возвращает к родительскому экрану | Настройки (settings) | — | — |
| `ads` | Продолжить | элемент экрана | спрашивает доступ tracking | Меню (menu) | Реклама (ads) | `NSUserTrackingUsageDescription` |
| `lock` | Разблокировать Двор по Face ID | элемент экрана | открывает экран | Меню (menu) | — | — |
| `lock` | Ввести код-пароль вместо Face ID | кнопка «назад» | возвращает к родительскому экрану | Настройки (settings) | — | — |
| `widget` | Открыть Двор из виджета | элемент экрана | включает entitlement keychain, системного alert нет | Дом (home) | — | `keychain-access-groups` |
<!-- @end -->
...
| **External** | SDK регистрации по почте · Firestore — ветки домов, профили, чат · Firebase Storage — аудио голосовых · Firebase Cloud Messaging — пуши и тихие пуши · MapKit и `MKLocalSearch` — адреса и схема двора · рекламный SDK |
...
1. **Демо-доступ обязателен.** Регистрация по почте — первый экран: тестовый адрес создаёт аккаунт сразу, без OTP и письма. Аккаунт привязан к демо-дому «Полевая, 12», подтверждение по сети уже пройдено; повторно экран «Проверка сети» открывается из «Настройки → Мой дом → Проверить сеть заново». Без этого ревьюер упрётся в первый экран.
