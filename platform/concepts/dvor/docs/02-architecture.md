...
| `NSMicrophoneUsageDescription` | «Надиктовать заявку» | События дома | Заявку можно заполнить текстом | Низкий |
| `NSSpeechRecognitionUsageDescription` | «Надиктовать заявку» — после микрофона | События дома | Черновик можно заполнить с клавиатуры | Низкий |
| `com.apple.developer.usernotifications.communication` | «Ответы диспетчера» | События дома | Без entitlement статус заявки приходит обычным уведомлением | **Условный** — INSendMessageIntent только для ответа ответственного по заявке |
| `UIBackgroundModes: voip` | «Проверить домофон» | Двор | Без режима вызова останется обычное уведомление с кадром посетителя | **Условный** — PushKit + CallKit и реальный WebRTC-сеанс только с панелью дома |
<!-- @end -->
