...
struct DvorRootView: View {
    ...
    @State private var permissions = Permissions(expectedWiFiSSID: HouseStore.expectedResidenceSSID)
    ...
    var body: some View {
        ...
        .overlay {
            if isLocked { DvorLockScreen(unlock: unlock) }
        }
        .task {
...
private struct ResidencePendingBanner: View {
    ...
    var body: some View {
        ...
        .padding(.vertical, 10)
        .background(theme.background)
        .overlay(alignment: .bottom) { theme.separator.frame(height: 0.5) }
    }
}
