...
`,
  ...
  "credential-provider": {
    ...
    source: ({ productName, bundleId }) => `import AuthenticationServices
import Security
import UIKit
...
final class CredentialProviderViewController: ASCredentialProviderViewController {
    override func prepareCredentialList(for serviceIdentifiers: [ASCredentialServiceIdentifier]) {
        ...
        door.setTitle("Домофон · квартира 48", for: .normal)
        door.addTarget(self, action: #selector(selectDoor), for: .touchUpInside)
        let guest = UIButton(type: .system)
        guest.setTitle("Гостевая сеть · Dvor-Guest", for: .normal)
    ...
    private func complete(recordIdentifier: String?) {
        ...
        let user = record == "guest" ? "Dvor-Guest" : "Квартира 48"
    ...
    private static func password(for account: String) -> String? {
        let query: [String: Any] = [
            ...
            kSecAttrService as String: "${bundleId}.house-access",
...
`,
  ...
  widget: {
    ...
    source: ({ productName, slug }) => `import SwiftUI
import WidgetKit
...
struct ProductWidgetView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text("Актуальные данные доступны в приложении")
                .font(.caption)
