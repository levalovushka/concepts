...
export function compileNativeConcept(concept, options = {}) {
  ...
  if (strategy === "mimicry" && referenceProfile && referenceProfile.status !== "ready") {
    diagnostics.push(diagnostic(
      "design.reference.evidence-incomplete",
      `Reference profile ${referenceProfile?.id} cannot drive mimicry until its screenshot evidence is complete`,
      "native.design.referenceProfile",
    ));
  }
