...
```bash
node launcher/gen/gen-launcher.mjs
cd launcher/build
...
| Экраны | снимки из `native/build/<slug>/shots` |
...
Библиотека читается прямо из репозитория: путь к `platform/` задаётся в настройках,
`concept.json` — источник правды, дублирования данных нет.
