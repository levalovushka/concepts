...
final class Nav {
    ...
    init(arguments: [String] = ProcessInfo.processInfo.arguments) {
        guard arguments.contains("-capture:signed-in") else { return }
        signedIn = true
        homeConfirmed = true

        if let tabArgument = arguments.first(where: { $0.hasPrefix("-capture:tab=") }) {
            switch tabArgument.replacingOccurrences(of: "-capture:tab=", with: "") {
