...
/** Authored SwiftUI объявляет продуктовые переходы в concept.json, а не в HTML. */
function nativeGraph(spec) {
  const ids = new Set(spec.screens.map((screen) => screen.id));
  const edges = (spec.transitions || []).map((edge) => ({
    from: edge.from, to: edge.to, kind: edge.kind || 'go', label: edge.label || '',
  }));
  const problems = [];
  for (const edge of edges) {
    if (!ids.has(edge.from)) problems.push(`переход ${edge.from} → ${edge.to}: нет исходного экрана`);
    if (!ids.has(edge.to)) problems.push(`переход ${edge.from} → ${edge.to}: нет целевого экрана`);
  }
  ...
  for (const id of ids) if (!reached.has(id)) problems.push(`экран ${id} недостижим из старта ${spec.start}`);
...
export function buildUxSpec(slug) {
  ...
  const authoredNative = existsSync(join(ROOT, 'native-apps', slug));
  const sourceMarkup = authoredNative ? {} : readMarkup(slug, sourceSpec);
  const { spec, markup } = authoredNative
    ? { spec: structuredClone(sourceSpec), markup: sourceMarkup }
    : prepareEmailRegistration(sourceSpec, sourceMarkup);
  const graph = authoredNative ? nativeGraph(spec) : screenGraph(spec, markup);
  if (graph.problems?.length) {
    throw new Error(`UX-спека ${slug}: битый граф\n  · ${graph.problems.join('\n  · ')}`);
  }
  const actions = authoredNative ? [] : screenActions(spec, markup);
  ...
  const screens = spec.screens.map((screen) => {
    ...
    const required = authoredNative ? declared : screen.ui?.pattern === 'auth'
