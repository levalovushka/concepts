...
try {
  ...
  for (const url of conceptUrls) {
    ...
    assert.equal(await phoneScreens.locator(':is(.auth-app-icon,.auth-inline-brand)').count(), await phoneScreens.count(), `${slug}: на входе нет логотипа или брендового знака`);
