...
assert.equal(IPHONE_COMPOSITION.deviceTop, 1050, 'нижнее скругление и таббар телефона должны уходить за границу кадра');
assert.equal(IPHONE_COMPOSITION.copyOffset, 40, 'типографика должна быть оптически центрирована над телефоном');
assert.equal(IPHONE_COMPOSITION.headlineSize, 136);
assert.equal(IPHONE_COMPOSITION.bodySize, 60);
...
assert.equal(
  beautifyStoreCopy('Прогулки поблизости'),
  'Прогулки поблизости',
  'двухсловный заголовок должен сохранять возможность переноса',
);
