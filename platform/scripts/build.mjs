...
const STATUS_ICONS = ["cellular", "wifi", "battery"]
  .map((name) =>
    read(join(KERNEL, `status-${name}.svg`)).replace(
      "<svg ",
      `<svg class="status-${name}" `,
    ),
  )
  .join("");
...
const authBack =
  '<button class="unified-auth-back tap" data-back aria-label="Назад"><svg class="ios-back" viewBox="0 0 12 21" fill="none"><path d="M10.25 1.75L1.75 10.5l8.5 8.75" stroke="currentColor" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round"/></svg><span>Назад</span></button>';
...
const authField = ({
  ...
  value,
  autocomplete,
}) => `<label class="unified-auth-field">
      <span>${label}</span>
      <input type="${type}" value="${value}" autocomplete="${autocomplete}" aria-label="${label}">
    </label>`;

const dvorAuthHead = (title) =>
  `<div class="d-top"><button class="d-back unified-auth-back tap" data-back aria-label="Назад"><svg class="ios-back" viewBox="0 0 12 21" fill="none"><path d="M10.25 1.75L1.75 10.5l8.5 8.75" stroke="currentColor" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round"/></svg></button><div class="d-top-title sm">${title}</div></div>`;
...
const dvorAuthField = ({
  ...
  value,
  autocomplete,
}) => `<label class="d-field d-email-field d-auth-field">
...
const dvorAccountAuthScreens = (spec, target) => {
  const shell = (id, content, pattern = "") =>
    `<div class="screen d-page auth-dvor${pattern ? " unified-auth" : ""}" id="scr-${id}"${pattern ? ` data-pattern="${pattern}"` : ""}><div class="body-scroll d-scroll d-safe d-col">${content}</div><div class="home-ind" aria-hidden="true"></div></div>`;
  const phone = dvorAuthField({
    label: "Номер телефона",
    type: "tel",
    value: "+7 900 123-45-67",
    autocomplete: "tel",
  });
  const password = dvorAuthField({
    label: "Пароль",
    type: "password",
    value: "sotki2026",
    autocomplete: "current-password",
  });
  const newPassword = dvorAuthField({
  ...
  return {
    password: shell(
      "password",
      `${dvorAuthHead("Пароль")}<div class="d-lead">Для аккаунта +7 900 123-45-67</div><div class="d-card pad">${password}</div><div class="push"></div><div class="d-cta"><button class="d-btn tap" data-primary data-auth-target data-go="${target}">Войти</button><button class="d-auth-link tap" data-toast="Ссылка для восстановления отправлена">Забыли пароль?</button></div>`,
      "auth",
    ),
    register: shell(
      "register",
      `${dvorAuthHead("Создать аккаунт")}<div class="d-lead">Номер нужен для входа и восстановления доступа</div><div class="d-card pad">${phone}<div class="d-note mute"><svg class="ico-svg"><use href="#i-info"/></svg><span>Приложением можно пользоваться и без аккаунта</span></div></div><div class="push"></div><div class="d-cta"><button class="d-btn tap" data-primary data-go="registerpassword">Далее</button><button class="d-auth-link tap" data-go="phone">Уже есть аккаунт</button></div>`,
      "auth",
    ),
    registerpassword: shell(
      "registerpassword",
      `${dvorAuthHead("Придумайте пароль")}<div class="d-lead">Для аккаунта +7 900 123-45-67</div><div class="d-card pad">${newPassword}</div><div class="push"></div><div class="d-cta"><button class="d-btn tap" data-primary data-auth-target data-go="${target}">Создать аккаунт</button>${authLegalFooter(spec)}</div>`,
      "auth",
    ),
    account: shell(
      "account",
      `${dvorAuthHead("Профиль и аккаунт")}<div class="d-sec">Вход</div><section class="d-card"><div class="d-row"><span class="d-bul" aria-hidden="true"><svg class="ico-svg"><use href="#i-phone"/></svg></span><div class="d-row-main"><div class="d-row-title">+7 900 123-45-67</div><div class="d-row-sub">Номер для входа</div></div></div><button class="d-row tap" data-go="password"><span class="d-bul" aria-hidden="true"><svg class="ico-svg"><use href="#i-lock"/></svg></span><span class="d-row-main"><span class="d-row-title">Изменить пароль</span><span class="d-row-sub">Пароль аккаунта</span></span><span class="d-caret"><svg class="ico-svg"><use href="#i-chevron-right"/></svg></span></button></section><div class="d-sec">Управление</div><section class="d-card"><button class="d-row tap" data-go="phone"><span class="d-row-main"><span class="d-row-title">Выйти из аккаунта</span><span class="d-row-sub">Данные на этом устройстве останутся</span></span></button><button class="d-row d-account-danger tap" data-primary data-go="deleteaccount"><span class="d-row-main"><span class="d-row-title">Удалить аккаунт</span><span class="d-row-sub">Профиль и связанные данные будут удалены</span></span><span class="d-caret"><svg class="ico-svg"><use href="#i-chevron-right"/></svg></span></button></section>`,
    ),
    deleteaccount: shell(
      "deleteaccount",
      `${dvorAuthHead("Удалить аккаунт?")}<div class="d-lead">Профиль и связанные с ним данные будут удалены безвозвратно</div><div class="d-card pad"><div class="d-note warn"><svg class="ico-svg"><use href="#i-circle-alert"/></svg><span>Дом, публикации и настройки на этом устройстве останутся, но не будут привязаны к профилю</span></div></div><div class="push"></div><div class="d-cta"><button class="d-btn danger unified-delete-confirm tap" data-primary data-go="phone">Удалить аккаунт</button><button class="d-btn quiet mt8 tap" data-back>Отмена</button></div>`,
      "auth",
    ),
  };
};
...
const accountAuthScreens = (spec, target, light = true, sourceClasses = "") => {
  ...
  return {
    ...
    account: `<div class="${rootClass}" id="scr-account"><div class="unified-auth-body">${authBack}
      <h1 class="unified-account-title">Аккаунт</h1><div class="unified-account-card"><div><span>Телефон</span><strong>+7 900 123-45-67</strong></div>
    ...
    deleteaccount: `<div class="${rootClass}" id="scr-deleteaccount"><div class="unified-auth-body">${authBack}
      <h1>Удалить аккаунт?</h1><p class="unified-auth-lede">Профиль и связанные с ним данные будут удалены<br>Это действие нельзя отменить</p>
...
function adaptRegistrationScreen(source, target, spec) {
  let out = source.replace(
    /id="scr-phone"/,
    'id="scr-phone" data-pattern="auth"',
  );
  out = out.replace(
    /<(button|div)(?=[^>]*(?:class="[^"]*(?:google|auth-apple)[^"]*"|data-activate="applesignin\|[^"]+"))[^>]*>[\s\S]*?<\/\1>/gi,
    "",
  );
  out = out.replace(
    /<([a-z]+)([^>]*data-go="code"[^>]*)>[\s\S]*?<\/\1>/,
    (match, tag, attrs) => {
      const clean = attrs
        .replace(/\sdata-go="code"/, "")
        .replace(/\sdata-primary(?:="[^"]*")?/, "");
  ...
  out = out
    ...
    .replace(
      /type="text" value="\+?7?\s?900[^\"]*"/g,
      'type="email" value="alex@inbox.ru"',
    )
    ...
    .replace(
      /<input([^>]*aria-label="Электронная почта"[^>]*)>/g,
      (match, attrs) => {
    ...
    .replace(
      /Пришлём код в (?:SMS|письме)\.[^<]*/g,
      "Почта создаст аккаунт сразу, без OTP и подтверждения.",
    )
    .replace(
      /Код придёт письмом\.[^<]*/g,
      "Аккаунт создастся сразу; письмо ждать не нужно.",
    )
  ...
  if (spec.slug !== "dvor") {
    out = out.replace(
      /<([a-z]+)([^>]*class="[^"]*(?:auth-legal|db-legal|lk-auth-note|lx-legal2|sc-legal|tl-auth-legal|t-footnote)[^"]*"[^>]*)>[\s\S]*?<\/\1>/gi,
      "",
    );
    if (/class="auth-links"/.test(out)) {
      out = out.replace(
        /<div class="auth-links">[\s\S]*?<\/div>/,
      ...
      if (spec.slug === "set") {
        out = out.replace(
          '</small></div></div><div class="home-ind"',
          `</small>${footer}</div></div><div class="home-ind"`,
        );
      } else {
        out = out.replace(
          '<div class="home-ind"',
          `${footer}<div class="home-ind"`,
        );
      }
    }
  }
...
const removeClassElement = (html, className) =>
  html.replace(
    new RegExp(
      `<([a-z]+)([^>]*class="[^"]*\\b${className}\\b[^"]*"[^>]*)>[\\s\\S]*?<\\/\\1>`,
      "gi",
...
function customPhoneAuthScreen(source, target, spec) {
  ...
  legalClasses.forEach((className) => {
    out = removeClassElement(out, className);
  });
  ...
  out = out
    ...
    .replace(/почта нужна/g, "номер нужен")
    .replace(
      /Пришлём код в письме\.[^<]*/g,
      "Пароль вводится на следующем экране.",
    )
    .replace(
      /Код придёт письмом\.[^<]*/g,
      "Пароль вводится на следующем экране.",
    ...
    .replace(
      /value="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}"/gi,
      'value="+7 900 123-45-67"',
    )
...
function applyLegalSafeguards(spec, sourceMarkup) {
  ...
  if (spec.slug === "tails") {
    ...
    markup.lock = markup.lock.replace(
    ...
    markup.vetnote = markup.vetnote
      .replace(">Заметка о самочувствии<", ">Наблюдение владельца<")
      .replace(
        '<div class="body-scroll tl-list">',
        '<div class="body-scroll tl-list"><div class="tl-fallback" style="display:block;background:#eef6ff;color:#435267">Это личные записи владельца, а не диагноз или рекомендация по лечению. При симптомах обратитесь к ветеринару.</div>',
      );
    markup.vaccine = markup.vaccine
      .replace(
        '<div class="tl-section-head tl-vaccine-next-head">',
        '<p class="tl-vaccine-legal">Схему вакцинации и препараты подтверждает ветеринар.</p><div class="tl-section-head tl-vaccine-next-head">',
      )
      .replace(
        ">Надиктовать заметку о самочувствии<",
        ">Добавить наблюдение владельца<",
      )
      .replace(
        "доза Bravecto считается от 20 до 40 кг",
        "вес записан на приёме; назначения хранит клиника",
      );
  }
  ...
  if (spec.slug === "double") {
    ...
    if (progress) {
      ...
      if (progress.ui?.primaryAction === "Напоминать о серии")
        progress.ui.primaryAction = "Напомнить о практике";
    }
...
export function prepareEmailRegistration(sourceSpec, sourceMarkup) {
  ...
  const target =
    spec.auth?.entryTarget ||
    firstGoOutsideAuth(sourceMarkup.code) ||
  ...
  spec.permissions = spec.permissions.filter(
    (permission) => permission.key !== "applesignin",
  );

  const sourcePhone = spec.screens.find((screen) => screen.id === "phone");
  const accountSurfaceId =
    ["settings", "menu", "profile"].find((id) =>
      sourceSpec.screens.some((screen) => screen.id === id),
    ) ||
    (spec.slug === "ptitsy"
      ? "season"
      : spec.slug === "volna"
  ...
  const sourceAuthClasses =
    ...
    "";
  const cases =
    spec.uiContractVersion >= 3
      ? [
          { kind: "typical", example: "Корректный российский номер телефона" },
          { kind: "stress", example: "Номер вставлен с пробелами и скобками" },
          {
            kind: "failure",
            example: "Номер или пароль неверен либо сеть недоступна",
          },
        ]
      : [
          "Корректный российский номер телефона",
          "Номер вставлен с пробелами и скобками",
          "Номер или пароль неверен либо сеть недоступна",
        ];
  const authScreen = (id, title, purpose, primaryAction, parent = null) => ({
    ...(id === "phone" ? sourcePhone || {} : {}),
    id,
    title,
    type: id === "phone" ? "старт, без таб-бара" : "push, без таб-бара",
    light: authLight,
    ...(parent ? { parent } : {}),
    meta: "Телефон и пароль · аккаунт опционален",
    ui: {
      pattern: id === "account" || id === "deleteaccount" ? "account" : "auth",
      navigation: "push",
      purpose,
      primaryAction,
      hierarchy: {
        primary: title,
        secondary: "Альтернативное действие и понятный возврат",
      },
      states:
        id === "account" || id === "deleteaccount"
          ? [...CONTENT_STATES]
          : [...FORM_STATES],
      density: "low",
      contentCases: cases,
  ...
  const generatedScreens = [
    ...
    authScreen(
      "password",
      "Пароль",
      "Ввести пароль отдельно от номера телефона",
      "Войти",
      "phone",
    ),
    authScreen(
      "register",
      "Создать аккаунт",
      "Начать отдельную ветку регистрации по номеру телефона",
      "Далее",
      "phone",
    ),
    authScreen(
      ...
      "Создать пароль отдельно от ввода номера",
      "Создать аккаунт",
      "register",
    ),
    authScreen(
      "account",
      "Аккаунт",
      "Управлять сессией и удалением аккаунта",
      "Удалить аккаунт",
      "password",
    ),
    authScreen(
      "deleteaccount",
      "Удаление аккаунта",
      "Подтвердить необратимое удаление аккаунта",
      "Удалить аккаунт",
      "account",
    ),
  ];
  spec.screens = spec.screens.filter(
    (screen) =>
      !AUTH_SCREENS.includes(screen.id) && !LEGACY_AUTH.has(screen.id),
  );
  spec.screens.unshift(...generatedScreens);
  spec.screens.forEach((screen) => {
    if (LEGACY_AUTH.has(screen.parent)) screen.parent = "phone";
  });
  spec.start = "phone";
  ...
  if (spec.product?.verticalSlice) {
    ...
    slice.entry = "phone";
    slice.action = replacement(slice.action);
    slice.result = replacement(slice.result);
    if (slice.action === slice.entry) slice.action = target;
    if (slice.result === slice.action || slice.result === slice.entry) {
      slice.result =
        spec.screens.find(
          (screen) => ![slice.entry, slice.action].includes(screen.id),
        )?.id || slice.result;
    }
  }
  if (spec.positioning?.evidenceScreens) {
    spec.positioning.evidenceScreens = [
      ...new Set(spec.positioning.evidenceScreens.map(replacement)),
    ];
    for (const screen of spec.screens) {
      if (spec.positioning.evidenceScreens.length >= 3) break;
      if (!spec.positioning.evidenceScreens.includes(screen.id))
        spec.positioning.evidenceScreens.push(screen.id);
    }
  }
  for (const row of spec.positioning?.referenceEvidence || [])
    row.screen = replacement(row.screen);
  ...
  spec.prototypes = (spec.prototypes || []).map((prototype) => {
    ...
    const startsWithAuth =
      prototype.start === "phone" || LEGACY_AUTH.has(prototype.start);
    const carriesAuth = sourceScreens.some(
      (id) => id === "phone" || LEGACY_AUTH.has(id),
    );
    let remaining = sourceScreens.filter(
      (id) => !AUTH_SCREENS.includes(id) && !LEGACY_AUTH.has(id),
    );
    ...
    if (carriesUnifiedAuth) {
      if (!remaining.includes(firstProductScreen))
        remaining.push(firstProductScreen);
    ...
    const normalized = {
      ...
      start,
      screens: remaining,
      stops: (prototype.stops || []).filter(
        (id) => !LEGACY_AUTH.has(id) && id !== "phone",
      ),
    };
    if (
      carriesUnifiedAuth ||
      (accountSurfaceId && remaining.includes(accountSurfaceId))
    )
      normalized.authTarget = firstProductScreen;
    if (prototype.hero) {
    ...
    if (!prototype.hero && prototype.id === "signin") {
      ...
      normalized.note =
        "Телефон и пароль вводятся отдельно; аккаунт можно пропустить или удалить";
    }
    return normalized;
  });
  ...
  if (
    ...
    !spec.appStore.privacy.some((row) => /Phone Number/i.test(row.apple || ""))
  ) {
    spec.appStore.privacy.push({
      type: "Номер телефона",
  ...
  for (const permission of spec.permissions || []) {
    if (permission.key === "keychain" && permission.grounding) {
      permission.grounding = permission.grounding.replace(
        /входа по номеру/gi,
        "сессии аккаунта",
      );
    }
  ...
  let markup = {
    ...sourceMarkup,
    ...accountAuthScreens(spec, target, authLight, sourceAuthClasses),
  };
  ...
  if (accountSurfaceId && markup[accountSurfaceId]) {
    markup[accountSurfaceId] = markup[accountSurfaceId]
      .replace(/[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}/gi, "+7 900 123-45-67")
      .replaceAll(
        "вход по почте, код приходит письмом",
        "вход по номеру и паролю",
      )
      .replaceAll(
        "Вход с Apple привязан к этому аккаунту",
        "Пароль можно изменить в управлении аккаунтом",
      )
      .replaceAll("Привязать вход с Apple", "Изменить пароль")
      .replaceAll("Вход с Apple", "Пароль аккаунта")
      .replaceAll(
        "не привязан · привяжите, чтобы входить без письма",
        "используется для входа после номера телефона",
      )
    ...
    if (
      (accountSurfaceId === "settings" || accountSurfaceId === "menu") &&
      !/data-go="account"/.test(markup[accountSurfaceId])
    ) {
      if (spec.slug === "dvor" && accountSurfaceId === "settings") {
        markup.settings = markup.settings.replace(
          '<div class="d-sec">Мой дом</div>',
          `${accountEntry}<div class="d-sec">Мой дом</div>`,
        );
      } else
        markup[accountSurfaceId] = /<div class="body-scroll[^>]*>/.test(
          markup[accountSurfaceId],
        )
          ? markup[accountSurfaceId].replace(
              /(<div class="body-scroll[^>]*>)/,
              `$1${accountEntry}`,
            )
          : markup[accountSurfaceId].replace(
              '<div class="home-ind"',
              `${accountEntry}<div class="home-ind"`,
            );
    } else if (accountSurfaceId === "profile") {
      markup.profile = markup.profile.replace(
        /<(button|div)([^>]*)>\s*<svg([^>]*)><use href="#i-settings"/,
        (match, tag, attrs, svgAttrs) => {
          const clean = attrs
            .replace(/\sdata-toast="[^"]*"/, "")
            .replace(/\sdata-go="[^"]*"/, "");
      ...
      markup.profile = markup.profile.replace(
        /<(button|div)([^>]*aria-label="Настройки"[^>]*)>/,
        (match, tag, attrs) => {
          const clean = attrs
            .replace(/\sdata-toast="[^"]*"/, "")
            .replace(/\sdata-go="[^"]*"/, "");
      ...
      if (!/data-go="account"/.test(markup.profile))
        markup.profile = markup.profile.replace(
          'data-go="phone" aria-label=',
          'data-go="account" aria-label=',
        );
      if (
        !/data-go="account"/.test(markup.profile) &&
        /class="settings-list"/.test(markup.profile)
      ) {
        markup.profile = markup.profile.replace(
          '<div class="settings-list">',
          '<div class="settings-list"><button data-go="account"><span class="setting-icon"><svg><use href="#i-user"/></svg></span><span><strong>Профиль и аккаунт</strong><small>Телефон, выход и удаление</small></span><svg><use href="#i-chevron-right"/></svg></button>',
        );
      }
      if (
        !/data-go="account"/.test(markup.profile) &&
        ...
        markup.profile = markup.profile.replace(
          '<div class="list-stack tight">',
          `<div class="list-stack tight">${accountEntry}`,
        );
      }
      if (!/data-go="account"/.test(markup.profile) && spec.slug === "seans") {
        markup.profile = markup.profile.replace(
          '</div>\n  </div>\n\n  <div class="body-scroll',
          '<div class="se-act tap" data-go="account" aria-label="Аккаунт"><svg class="ico-svg"><use href="#i-settings"/></svg></div></div>\n  </div>\n\n  <div class="body-scroll',
        );
      }
    } else if (spec.slug === "ptitsy") {
      markup.season = markup.season.replace(
        '<div class="icon-btn tap" data-go="releases"',
        '<div class="icon-btn tap" data-go="account" aria-label="Аккаунт"><svg class="ico-svg"><use href="#i-settings"/></svg></div><div class="icon-btn tap" data-go="releases"',
      );
    } else if (spec.slug === "volna") {
      markup.library = markup.library.replace(
        '<button class="vl-icon tap" data-go="import"',
        '<button class="vl-icon tap" data-go="account" aria-label="Аккаунт"><svg><use href="#i-settings"/></svg></button><button class="vl-icon tap" data-go="import"',
      );
    }
  }
  delete markup.code;
  delete markup.codefail;
  ...
  for (const screen of spec.screens) {
    ...
    const isForm =
      ...
      /<(?:form|textarea|select)\b|contenteditable|<input\b(?![^>]*type="search")/i.test(
        html,
      );
    const extraStates = (screen.ui?.states || []).filter(
      (state) => state === "success",
    );
    screen.ui = {
      ...(screen.ui || {}),
      states: isForm
        ? [...FORM_STATES]
        : [...new Set([...CONTENT_STATES, ...extraStates])],
    };
  }
  return { spec, markup };
}
...
${spec.screens
  .map((s) => {
    const on =
      spec.permissions
        .filter((p) => p.screen === s.id)
        .map((p) => p.key + (p.activate ? " (activate)" : ""))
        .join(", ") || "—";
...
const docsLinks = (spec, dir) =>
  (spec.docs || [])
    .map(
      (d, index) =>
        `<button type="button" data-doc="${docId(d.file)}" aria-pressed="${index === 0}">${esc(d.label)}</button>`,
    )
    .join("\n      ") +
  `\n      <a class="zip" href="docs/${spec.slug}-docs.zip" download>Скачать все документы (ZIP)</a>` +
  (existsSync(
    join(dir, "assets", "app-store", `${spec.slug}-app-store-assets.zip`),
  )
...
const docsContent = (spec, dir) =>
  (spec.docs || [])
    .map((doc, index) => {
      const file = join(dir, "docs", doc.file);
      if (!existsSync(file))
        throw new Error(`${spec.slug}: нет документа ${doc.file}`);
...
const titleOf = (spec, id) =>
  spec.screens.find((s) => s.id === id)?.title || id;
...
function packDocs(slug, dir) {
  ...
  try {
    ...
    execFileSync(
      "zip",
      ["-q", zip, ...readdirSync(docs).filter((f) => f.endsWith(".md"))],
      { cwd: docs },
    );
...
const productUiContractCss = (spec) => {
  ...
  const customAuthMetrics = {
    double: ["32px", "36px", "650", "13px"],
    dvor: ["24px", "29px", "700", "8px"],
    liga: ["28px", "32px", "750", "12px"],
    looks: ["29px", "33px", "500", "12px"],
    radius: ["28px", "34px", "650", "12px"],
    scene: ["31px", "35px", "800", "12px"],
    set: ["32px", "36px", "800", "14px"],
    shellac: ["34px", "37px", "790", "12px"],
...
const stripTabbar = (markup) => {
  const re =
    /<(div|nav)\b[^>]*class="[^"]*(?:\btabbar\b|[a-z]+-tabs\b)[^"]*"[^>]*>/g;
  let out = markup,
    m;
  while ((m = re.exec(out))) {
    const tag = m[1];
...
const deviceShell = (
  ...
  screensHtml,
  ledgerId,
) => `<div class="device" id="pr-${proto.id}" data-proto="${proto.id}" data-start="${proto.start}"${ledgerId ? ` data-ledger="${ledgerId}"` : ""}>
          <div class="status">
            <span>9:41</span>
...
export function build(slug, { outDir } = {}) {
  ...
  if (graph.problems.length) {
    throw new Error(
      `${slug}: карта экранов не сходится —\n  · ` +
        graph.problems.join("\n  · "),
    );
  }
  ...
  for (const p of protos) {
    ...
    if (!own.has(p.start))
      throw new Error(
        `${slug}/${p.id}: стартовый экран ${p.start} не входит в сценарий`,
      );
    for (const s of stops)
      if (!own.has(s))
        throw new Error(`${slug}/${p.id}: тупик ${s} не входит в сценарий`);
    if (stops.has(p.start))
      throw new Error(`${slug}/${p.id}: стартовый экран не может быть тупиком`);
    const dangling = new Set();
    for (const id of p.screens) {
      if (!markup[id])
        throw new Error(`${slug}/${p.id}: экрана ${id} нет в спеке`);
      ...
      for (const t of targetsIn(body))
        if (!own.has(t)) dangling.add(`${id}→${t}`);
    }
    if (dangling.size)
      throw new Error(
        `${slug}/${p.id}: переходы за пределы сценария — ${[...dangling].join(", ")}`,
      );
  }
  ...
  const heroDevice =
    deviceShell(
      hero,
      hero.screens.map((id) => screenFor(hero, id, markup[id])).join("\n\n"),
      "perms",
    ) +
    "\n        " +
    controls;
  ...
  const styles = existsSync(join(dir, "styles.css"))
    ? read(join(dir, "styles.css"))
    : "";
  const rawSections = existsSync(join(dir, "sections.html"))
  ...
  const grab = (name) => {
    const m = rawSections.match(
      new RegExp(`<!-- @overview:${name} -->([\\s\\S]*?)<!-- @end -->`),
    );
    return m ? m[1].trim() : "";
  };
  const sections = rawSections
    .replace(/<!-- @overview:[a-z]+ -->[\s\S]*?<!-- @end -->/g, "")
    .trim();
  const viewToggle =
    spec.targetSet === "vk-video"
      ? `<div class="view-switch" data-view-switch role="group" aria-label="Размер устройства">
  ...
  const html = fill(read(join(KERNEL, "page.html")), {
    ...
    APP_ICON_TOPBAR: useIconPlaceholder
      ...
      : hasAppIcon
        ? `<img class="topbar-app-icon" src="assets/app-icon.png" alt="">`
        : "",
    ...
    CSS: [
      read(join(KERNEL, "base.css")),
      brandCss(spec.brand),
      styles,
      read(join(KERNEL, "tablet.css")),
      productUiContractCss(spec),
    ].join("\n"),
  ...
  if (left)
    throw new Error(
      `${slug}: незаполненные слоты — ${[...new Set(left)].join(", ")}`,
    );
