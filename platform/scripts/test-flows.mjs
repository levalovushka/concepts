...
async function run(slug) {
  ...
  const { spec, markup } = prepareEmailRegistration(
    sourceSpec,
    readMarkup(slug, sourceSpec),
  );
  const file = join(DIST, slug, "index.html");
  if (!existsSync(file))
    throw new Error(
      `${slug}: сначала соберите — node scripts/build.mjs ${slug}`,
    );
  ...
  const errs = [];
  page.on("console", (m) => {
    if (m.type() === "error" || /нет экрана|нет доступа/.test(m.text())) {
  ...
  const hero =
    (spec.prototypes || []).find((p) => p.hero) || (spec.prototypes || [])[0];
  const H = `#pr-${hero.id}`;
  const cur = () =>
    page.evaluate(
      (h) => document.querySelector(h + " .screen.is-on")?.dataset.screen,
      H,
    );
  const alertOn = () =>
    page.evaluate(
      (h) => document.querySelector(h + " .sysask").classList.contains("is-on"),
      H,
    );
  const answer = (a) => page.click(`${H} [data-answer="${a}"]`);
  const reset = async () => {
    await page.click(`${H} ~ .controls [data-act="reset"]`);
  ...
  ok(
    "журнал пуст на старте",
    (await page.$$('#perms .perm:not([data-state="idle"])')).length === 0,
  );
  ok("на старте нет промпта", !(await alertOn()));
  ok(
    "status bar не рисует свою подложку",
    await page.evaluate((h) => {
      const color = getComputedStyle(
        document.querySelector(h + " .status"),
      ).backgroundColor;
      return color === "transparent" || /rgba\([^)]*,\s*0\s*\)$/.test(color);
    }, H),
  );
  ok(
    "auth не получает чужую подложку под status bar",
    await page.evaluate((h) => {
  ...
  for (const sourcePrototype of sourceSpec.prototypes || []) {
    if (
      sourcePrototype.hero ||
      sourcePrototype.start === "phone" ||
      ["code", "codefail"].includes(sourcePrototype.start)
    )
      continue;
    const effectivePrototype = spec.prototypes.find(
      (prototype) => prototype.id === sourcePrototype.id,
    );
    ok(
      `${sourcePrototype.id}: сценарий сохраняет старт «${sourcePrototype.start}»`,
      effectivePrototype?.start === sourcePrototype.start,
    );
  }
  const externalAuth = await page.evaluate((h) => {
    const auth = document.querySelector(h + ' [data-screen="phone"]');
    return !!auth?.querySelector(
      '[class*="google"], .auth-apple, [data-activate^="applesignin|"]',
    );
  }, H);
  ok(
    "вход не предлагает внешних провайдеров",
    !externalAuth && !spec.permissions.some((p) => p.key === "applesignin"),
  );
  ok(
    "номер и пароль находятся на разных экранах",
    !!(await page.$(`${H} [data-screen="phone"] input[type="tel"]`)) &&
  ...
  ok(
    ...
    !!(await page.$(
      `${H} [data-screen="account"] [data-go="deleteaccount"]`,
    )) &&
      !!(await page.$(
        `${H} [data-screen="deleteaccount"] .unified-delete-confirm`,
      )),
  );
  ok(
    "у формы нет состояний offline и permission",
    await page.evaluate((h) => {
      const controls = document
        .querySelector(h)
        ?.parentNode.querySelector(":scope > .controls");
      return (
        controls?.querySelector('[data-state="offline"]')?.hidden &&
  ...
  ok(
    "empty формы = пустой input без оверлея",
    await page.evaluate((h) => {
      const phone = document.querySelector(h + ' [data-screen="phone"]');
      return (
        phone?.querySelector('input[type="tel"]')?.value === "" &&
        !document.querySelector(h + " .prototype-state.is-on")
      );
    }, H),
  );
  await page.click(`${H} ~ .controls [data-state="error"]`);
  ok(
    "error формы = невалидный input и inline-текст без оверлея",
    await page.evaluate((h) => {
      const phone = document.querySelector(h + ' [data-screen="phone"]');
      return (
        phone?.querySelector('input[aria-invalid="true"]') &&
        phone.querySelector("[data-prototype-form-message]") &&
        !document.querySelector(h + " .prototype-state.is-on")
      );
    }, H),
  );
  ...
  ok(
    "loading формы остаётся внутри формы",
    await page.evaluate((h) => {
      const phone = document.querySelector(h + ' [data-screen="phone"]');
      return (
        phone?.querySelector(
          '[aria-disabled="true"].prototype-button-loading',
        ) && !document.querySelector(h + " .prototype-state.is-on")
      );
    }, H),
  );
  ok(
    "spinner loading-кнопки свёрстан в строку и не наезжает на текст",
    await page.evaluate((h) => {
      const button = document.querySelector(
        h + ' [data-screen="phone"] .prototype-button-loading',
      );
  ...
  const contentScreen = spec.screens.find(
    (screen) =>
      ![
        "phone",
        "password",
  ...
  if (contentScreen) {
    ...
    ok(
      `${contentScreen}: контенту доступны offline и permission`,
      await page.evaluate((h) => {
        const controls = document
          .querySelector(h)
          ?.parentNode.querySelector(":scope > .controls");
    ...
    await page.click(`${H} ~ .controls [data-state="loading"]`);
    ok(
      `${contentScreen}: loading контента показан на самом экране`,
      await page.evaluate((h) => {
        const state = document.querySelector(h + " .prototype-state");
        return (
          state?.classList.contains("is-on") &&
  ...
  const missing = await page.evaluate(
    ({ ids, h }) =>
      ids.filter((id) => !document.querySelector(h + ` [data-screen="${id}"]`)),
    { ids: spec.screens.map((s) => s.id), h: H },
  );
  ok(
    `все ${spec.screens.length} экранов спеки есть в разметке` +
      (missing.length ? ` → нет: ${missing}` : ""),
    !missing.length,
  );
  ...
  const broken = await page.evaluate((h) => {
    ...
    const known = new Set(
      [...root.querySelectorAll("[data-screen]")].map((e) => e.dataset.screen),
    );
  ...
  ok(
    "нет битых переходов" + (broken.length ? ` → ${broken}` : ""),
    !broken.length,
  );
  ...
  const reachable = await page.evaluate((h) => {
    ...
    root
      ...
      .forEach((e) => s.add(e.dataset.activate.split("|")[0]));
    return [...s];
  }, H);
  ...
  ok(
    "в разметке нет доступов вне спеки" + (stray.length ? ` → ${stray}` : ""),
    !stray.length,
  );
  ...
  const routePermissions = await page.evaluate(
    ({ h, start, parent }) => {
      const root = document.querySelector(h);
      ...
      const add = (from, to) => {
        if (from && known.has(to)) graph[from].add(to);
      };

      for (const screen of screens) {
        const from = screen.dataset.screen;
        screen
          .querySelectorAll("[data-go],[data-jump]")
          .forEach((e) => add(from, e.dataset.go || e.dataset.jump));
        ...
        screen
          .querySelectorAll("[data-activate]")
          .forEach((e) => add(from, e.dataset.activate.split("|")[1]));
        screen
      ...
      for (const screen of screens) {
        ...
        screen.querySelectorAll("[data-ask]").forEach((e) => {
          e.dataset.ask
            .split("|")[0]
            .split("+")
            .forEach((key) => (triggers[key] ||= new Set()).add(id));
        });
      ...
      return {
        ...
        triggers: Object.fromEntries(
          Object.entries(triggers).map(([key, ids]) => [key, [...ids]]),
        ),
      };
    },
    {
      h: H,
      start: spec.start,
      parent: Object.fromEntries(spec.screens.map((s) => [s.id, s.parent])),
    },
  );
  for (const p of spec.permissions) {
    ...
    ok(
      `${p.key}: UI-триггер достижим от «${spec.start}»` +
        (available.length
          ? ` через «${available.join(", ")}»`
          : ` → триггеры только на: ${triggerScreens.join(", ") || "нет"}`),
      available.length > 0,
    );
  }
  ...
  for (const p of spec.permissions) {
    ...
    const triggers = await page.evaluate(
      ({ k, h }) =>
        [...document.querySelectorAll(h + " [data-ask]")]
          ...
          .map((e) => ({
            screen: e.closest(".screen").dataset.screen,
            ask: e.dataset.ask,
          })),
      { k: p.key, h: H },
    );
    if (!triggers.length) {
      ok(`${p.key}: есть триггер запроса`, false);
      continue;
    }
    ...
    for (const t of triggers) {
      ...
      await goto(t.screen);
      await page.click(
        `${H} [data-screen="${t.screen}"] [data-ask="${t.ask}"]`,
      );
      await page.waitForTimeout(60);
      for (let i = 0; i < idx; i++) {
        await answer("grant");
      ...
      if (!(await alertOn())) {
        ok(`${p.key} с «${t.screen}»: показан alert`, false);
        continue;
      }
      await answer("deny");
      await page.waitForTimeout(80);
      ...
      const shown = await page.evaluate(
        ({ k, h }) => {
          ...
          const vis = (e) =>
            e &&
            !e.classList.contains("perm-hidden") &&
            e.offsetParent !== null;
          const note = [...scr.querySelectorAll("[data-show-denied]")].some(
            (e) =>
              e.dataset.showDenied
                .split(",")
                .map((s) => s.trim())
                .includes(k) && vis(e),
          );
          const sw = [...scr.querySelectorAll(`[data-switch="${k}"]`)].some(
  ...
  for (const chain of [...new Set(chains)]) {
    ...
    const scr = await page.evaluate(
      ({ c, h }) =>
        document.querySelector(h + ` [data-ask^="${c}|"]`).closest(".screen")
          .dataset.screen,
      { c: chain, h: H },
    );
    await goto(scr);
    ...
    ok(
      `цепочка ${chain}: отказ на ${first} не спрашивает ${second}`,
      asked === 1,
    );
  }
  ...
  if (p0) {
    const scr = await page.evaluate(
      ({ k, h }) => {
        const el = [...document.querySelectorAll(h + " [data-ask]")].find((e) =>
          e.dataset.ask.split("|")[0].split("+").includes(k),
        );
  ...
  for (const s of spec.screens) {
    ...
    const hasBack = await page.evaluate(
      ({ id, h }) =>
        !!document.querySelector(h + ` [data-screen="${id}"] [data-back]`),
      { id: s.id, h: H },
    );
    if (!hasBack) continue;
    await reset();
    ...
    ok(
      `«${s.title}» → назад ведёт на «${s.parent}»` +
        (landed === s.parent ? "" : ` → попали на ${landed}`),
      landed === s.parent,
    );
  }
  await reset();
  ...
  const small = await page.evaluate((h) => {
    ...
    document
      .querySelectorAll(
        `${h} [data-go],${h} [data-ask],${h} [data-back],${h} [data-activate],${h} [data-jump],${h} [data-toast]`,
      )
      .forEach((e) => {
        ...
        if (r.width && r.height < 44 && !e.classList.contains("tap"))
          bad.push((e.textContent || e.ariaLabel || "?").trim().slice(0, 24));
        if (!was) scr.classList.remove("is-on");
      });
    return bad;
  }, H);
  ok(
    "хит-таргеты ≥ 44pt" + (small.length ? ` → ${small.slice(0, 4)}` : ""),
    !small.length,
  );
  ok(
    "консоль чистая" + (errs.length ? ` → ${errs.slice(0, 3)}` : ""),
    !errs.length,
  );
    ...
    const plan = await page.evaluate(
      ({ h, start }) => {
        ...
        for (const s of screens) {
          ...
          s.querySelectorAll("[data-ask]").forEach((e) =>
            add(e.dataset.ask.split("|")[1], e),
          );
          s.querySelectorAll("[data-activate]").forEach((e) =>
            add(e.dataset.activate.split("|")[1], e),
          );
          s.querySelectorAll("[data-toast]").forEach((e) => {
        ...
        while (queue.length) {
          const from = queue.shift();
          for (const { to, sel } of edges[from] || [])
            if (!(to in prev)) {
              prev[to] = { from, sel };
              queue.push(to);
            }
    ...
    for (const perm of spec.permissions) {
      ...
      const granted = broke
        ...
        : await page.evaluate((t) => {
            const row = [...document.querySelectorAll("#perms .perm")].find(
              (e) => e.querySelector(".perm-name")?.textContent.trim() === t,
            );
...
await Promise.all(
  Array.from({ length: LANES }, async () => {
    while (next < slugs.length) {
      const i = next++;
      try {
        results[i] = await run(slugs[i]);
      } catch (e) {
        results[i] = [{ name: `прогон упал: ${e.message}`, pass: false }];
      }
    }
  }),
);
